...
export default function GeneratedQuestionCard({ question, onSave, onRemove, onEdit }) {
  ...
  const getStatusBadge = () => {
    if (question.error) {
      return <span className="px-2 py-1 text-xs font-semibold bg-error/20 text-error rounded-full">Error</span>;
    }
    if (question.saving) {
      return (
    ...
    if (question.saved) {
      return <span className="px-2 py-1 text-xs font-semibold bg-success/20 text-success rounded-full">✓ Saved</span>;
    }
    return <span className="px-2 py-1 text-xs font-semibold bg-bg-tertiary text-text-secondary rounded-full">Unsaved</span>;
  };

  // Type badge styling
  const getTypeColor = () => {
    switch (question.type) {
      case 'mcq': return 'bg-accent-primary/20 text-accent-primary';
      case 'short': return 'bg-accent-secondary/20 text-accent-secondary';
      case 'written': return 'bg-warning/20 text-warning';
    ...
    >
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <span className={`px-2 py-1 text-xs font-semibold rounded-full ${getTypeColor()}`}>
        ...
        {isMCQ && question.options && (
          <div className="space-y-2 mt-3">
            {(Array.isArray(question.options) ? question.options : JSON.parse(question.options || '[]')).map((option, idx) => {
              const correctAnswer = language === 'bn' ? question.answer_bn : question.answer;
              const isCorrect = option.startsWith(correctAnswer) || option === correctAnswer;
        ...
        {explanation && (
          <div className="bg-bg-primary border border-bg-tertiary rounded-lg p-3">
            <p className="text-[10px] font-mono uppercase text-text-tertiary mb-1">Explanation</p>
