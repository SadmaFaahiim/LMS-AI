...
export default function QuestionSelector({ selectedIds, onChange, disabled = false }) {
  ...
  const filteredQuestions = questions.filter(q => {
    const matchesSearch = !search || getLocalText(q, 'question_text').toLowerCase().includes(search.toLowerCase());
  ...
  const selectClass = "bg-bg-secondary border border-bg-tertiary rounded-lg px-4 py-2.5 text-sm text-text-primary focus:outline-none focus:border-accent-primary appearance-none cursor-pointer";
    ...
    <div className="relative">
      ...
      <button
        ...
        className={`w-full px-4 py-3 bg-bg-secondary border border-bg-tertiary rounded-lg text-left flex items-center justify-between transition-colors ${
          disabled ? 'opacity-50 cursor-not-allowed' : 'hover:border-accent-primary cursor-pointer'
        }`}
      >
        <div>
        ...
        <div className="absolute z-50 w-full mt-2 bg-bg-secondary border border-bg-tertiary rounded-xl shadow-2xl max-h-96 flex flex-col">
                  ...
                  return (
                    <label
                      ...
                      className={`flex items-start gap-3 p-3 rounded-lg cursor-pointer transition-colors ${
                        isSelected ? 'bg-accent-primary/10 border border-accent-primary/30' : 'hover:bg-bg-primary'
                      }`}
                    >
                      <input
                        type="checkbox"
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-text-primary line-clamp-2">{questionText}</p>
                        <div className="flex items-center gap-2 mt-1">
                          <span className={`text-[10px] font-mono uppercase px-1.5 py-0.5 rounded ${
                            question.type === 'mcq' ? 'bg-info/20 text-info' : 'bg-accent-secondary/20 text-accent-secondary'
                          }`}>
                            {question.type}
                          </span>
                          <span className="text-xs text-text-secondary font-mono">{question.marks} marks</span>
                          {question.difficulty && (
                            <span className="text-xs text-text-secondary">· {question.difficulty}</span>
                          )}
                        </div>
                      </div>
                    </label>
          ...
          <div className="p-3 border-t border-bg-tertiary flex items-center justify-between">
            <span className="text-xs text-text-secondary">
              {selectedIds.length} selected · {selectedQuestions.reduce((sum, q) => sum + (q.marks || 0), 0)} total marks
            </span>
            <button
