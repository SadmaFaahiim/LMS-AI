...
export default function QuestionCard({ question, onUpdate, onPublish, onReject }) {
  ...
  const handlePublish = async () => {
    try {
      await api.updateQuestion(question.id, { status: 'published', is_published: true });
      if (onUpdate) {
        onUpdate();
      }
    } catch (err) {
  ...
  const handleReject = async () => {
    ...
    try {
      await api.updateQuestion(question.id, { status: 'rejected', is_published: false });
      if (onUpdate) {
        onUpdate();
      }
    } catch (err) {
  ...
  const typeColor = isMCQ ? 'text-info bg-info/20' : 'text-accent-secondary bg-accent-secondary/20';
  const statusColor = {
  ...
  const options = isMCQ && question.options
  ...
  return (
    <div className="bg-bg-secondary border border-bg-tertiary rounded-xl overflow-hidden hover:border-text-tertiary transition-colors">
      ...
      >
        <span className={`text-[10px] font-mono uppercase tracking-widest px-2 py-0.5 rounded ${typeColor}`}>
          {isMCQ ? 'MCQ' : question.type}
        </span>
        <span className={`text-[10px] font-mono uppercase tracking-widest px-2 py-0.5 rounded ${statusColor}`}>
          {question.status}
        </span>
        ...
        <div className="flex items-center gap-2 text-xs text-text-secondary font-mono shrink-0">
          ...
          <span className="ml-2 text-accent-primary">{expanded ? '▲' : '▼'}</span>
        </div>
      </div>
      ...
      {expanded && (
        <div className="px-5 pb-5 border-t border-bg-tertiary">
          <div className="mt-4 space-y-3">
            <p className="text-sm text-text-primary leading-relaxed">{questionText}</p>

            {isMCQ && options.length > 0 && (
              <ul className="space-y-1.5">
                {options.map((opt, i) => {
                  const isCorrect = opt.startsWith(question.answer) || opt === question.answer;
                  return (
                    <li
                      key={i}
